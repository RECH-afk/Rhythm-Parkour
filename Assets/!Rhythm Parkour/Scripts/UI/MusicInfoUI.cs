...
/// <summary>
/// MusicInfo — окно с обложкой / названием / длительностью.
/// Анимация DOTween: сверху → середина (замедление у центра) → пауза → вниз.
/// Требуется TextMeshPro (как у тебя) + DOTween.
/// Повесь на панель (RectTransform) внутри Canvas. Назначь поля.
/// </summary>
public class MusicInfoUI : MonoBehaviour
{
    ...
    public RectTransform panelRect; // то что едет
    public Image coverImage;
    public Image coverBackground;
    public TextMeshProUGUI titleText;
    public TextMeshProUGUI durationText;
    public TextMeshProUGUI extraText; // опционально

    [Header("Тайминг")]
    public float showDelay = 0.3f;
    public float stayDuration = 2.2f; // сколько висит в середине
    ...
    [Header("DOTween")]
    public Ease slideInEase = Ease.OutCubic; // быстро сверху, замедление ближе к середине
    public Ease slideOutEase = Ease.InCubic; // ускорение вниз
    ...
    void PlayDOTween()
    {
        ...
        // небольшая подготовка — лёгкий скейл для сочности
        panelRect.localScale = Vector3.one * 0.96f;

        Sequence seq = DOTween.Sequence();
        // сверху → середина (замедление у середины за счёт OutCubic)
        ...
        seq.SetUpdate(true); // на паузе тоже тикает
        activeTween = seq;
    }
