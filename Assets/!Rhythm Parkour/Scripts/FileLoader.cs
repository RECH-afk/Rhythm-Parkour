...
namespace RKS.RhythmParkour
{
    ...
    public class FileLoader : RKSBehaviour
    {
        ...
        private IEnumerator LoadByType(string path, FileType type)
        {
            ...
            {
                case FileType.Audio:
                    using (var www = UnityWebRequestMultimedia.GetAudioClip(RkslStore.GetFileUri(path), GetAudioType(path)))
                    {
                ...
                case FileType.Photo:
                    using (var www = UnityWebRequestTexture.GetTexture(RkslStore.GetFileUri(path)))
                    {
                        yield return www.SendWebRequest();
                        if (www.result == UnityWebRequest.Result.Success)
                        {
        ...
        private AudioType GetAudioType(string path) => RkslStore.GetAudioType(path);
