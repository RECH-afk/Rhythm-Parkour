...
/// <summary>
/// Оценка ноты: Perfect / Great / Good / Miss.
/// </summary>
...
/// <summary>
/// Итоги прохождения уровня (снапшот на момент финиша).
/// </summary>
[Serializable]
public struct LevelResults
{
    ...
    public int maxCombo;
    public float flow; // Поток 0–100 на финише (вместо очков)
    public float accuracy; // 0..1
    public string rank;    // GODLIKE! / AWESOME / SO SO / BAD / TRASH
    public bool failed;
    public int groovePerfect; // действий идеально в бит
    public int grooveGood;    // действий хорошо в бит
    public int maxMissStreak;  // худшая серия миссов подряд

    public int Judged => count300 + count100 + count50 + countMiss;
}

/// <summary>
/// Счёт уровня: серия битов, Поток и точность, ранг.
/// Один на сцену (AutoCreate в IsGameScene). Сбрасывается в BeginLevel().
/// Препятствие = одна "нота": увернулся = Perfect, задел = Miss (максимум один мисс с препятствия).
/// Серия (комбо): +1 за каждый пережитый бит музыки, Miss обнуляет.
/// Нот тут нет дискретных QTE-моментов — игра непрерывная, поэтому серия тикает по битам.
/// </summary>
public class RhythmScoreManager : MonoBehaviour
{
    ...
    /// <summary>Сработало при любом джаджменте (для HUD-попапа).</summary>
    public event Action<HitJudgement, int> onJudgement; // judgement, combo после
    public event Action onScoreChanged;
    public event Action<LevelResults> onLevelFinished;
    /// <summary>Комбо перешагнуло очередной milestoneStep (для попапа/звука).</summary>
    public event Action<int> onComboMilestone; // combo
    ...
    void SubscribeBeats()
    {
        ...
        if (c == null) c = FindObjectOfType<Conductor>();
        if (c != null) { beatSrc = c; beatSrc.onBeat += OnBeatTick; }
    }
    ...
    /// <summary>Каждый пережитый бит музыки растит серию (если уровень идёт и игрок жив).</summary>
    void OnBeatTick()
    {
    ...
    /// <summary>Старт подсчёта для нового прохождения.</summary>
    ...
    /// <summary>Зарегистрировать уже вычисленный грейд.</summary>
    public void RegisterJudgement(HitJudgement j) => Register(j);

    /// <summary>Урон от препятствия = Miss, комбо сгорает.</summary>
    ...
    /// <summary>
    /// Действие в бит (прыжок/слайд/приземление): капает в Поток, комбо и точность не трогает.
    /// </summary>
    ...
    void Register(HitJudgement j)
    {
        ...
        switch (j)
        {
            ...
            case HitJudgement.Miss:
                countMiss++;
                combo = 0; // урон обнуляет серию битов
    ...
    /// <summary>Серия перешагнула очередной milestoneStep (для попапа/звука).</summary>
    void CheckMilestone()
    {
    ...
    /// <summary>Игрок погиб — дальше только финиш с TRASH.</summary>
    public void Fail()
    {
        isFailed = true;
    }

    /// <summary>Конец уровня: считает итоги и кидает событие для экрана результатов.</summary>
    ...
    /// <summary>Точность 0..1: взвешенные очки джаджментов / максимум. Miss всегда 0.</summary>
    ...
    /// <summary>Ранг: GODLIKE! / AWESOME / SO SO / BAD / TRASH. Провал = TRASH.</summary>
    public string Rank
    {
        get
        {
    ...
    public static Color RankColor(string rank)
    {
        switch (rank)
        {
            case "GODLIKE!": return new Color(1f, 0.84f, 0.25f);  // золото
            case "AWESOME": return new Color(0.35f, 0.95f, 0.45f); // зелёный
            case "SO SO": return new Color(1f, 0.95f, 0.35f);      // жёлтый
            case "BAD": return new Color(1f, 0.6f, 0.25f);         // оранж
            case "TRASH": return new Color(0.55f, 0.55f, 0.6f);    // серый
            default: return new Color(0.7f, 0.7f, 0.75f);          // неизвестный — серый
        }
    }
    ...
    public static Color JudgementColor(HitJudgement j)
    {
        switch (j)
        {
            case HitJudgement.Perfect300: return new Color(0.35f, 0.9f, 1f);  // циан как в osu 300
            case HitJudgement.Great100: return new Color(0.45f, 1f, 0.45f);   // зелёный
            case HitJudgement.Good50: return new Color(1f, 0.85f, 0.3f);      // жёлтый
            default: return new Color(1f, 0.3f, 0.3f);                        // красный
        }
    }
    ...
    {
        if (UnityEngine.SceneManagement.SceneManager.GetActiveScene().name != "IsGameScene") return;
        if (FindObjectOfType<RhythmScoreManager>() != null) return;
        var mgr = FindObjectOfType<RhythmParkourManager>();
        var go = mgr != null ? mgr.gameObject : new GameObject("RhythmScore (Auto)");
        if (go.GetComponent<RhythmScoreManager>() == null) go.AddComponent<RhythmScoreManager>();
    }
}
